...
public class SearchSale extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String state = request.getParameter("id_state");
		String dateTime = request.getParameter("id_saledate");
		...
		try {
			...
			if (saleall.equals("true")) {
				...
				sql = new String(
						"SELECT commodity.comno,comname,saleamount,saledate FROM sale,commodity WHERE sale.comno=commodity.comno AND stano=?");
			} else if (!saleno.equals("")) {
				params = new String[] { stano, saleno };
				sql = new String(
						"SELECT commodity.comno,comname,saleamount,saledate FROM sale,commodity WHERE sale.comno=commodity.comno AND stano=? AND saleno=?");
			} else {
				...
				/*
				 * if(!salesortno.equals("0")){ if(j==true){
				 * sql2=" AND sortno="+salesortno; }else{
				 * sql2=" sortno="+salesortno; j=true; } }
				 */
				if (j == true) {
				...
				sql = "SELECT commodity.comno,comname,saleamount,saledate FROM sale,commodity WHERE"
						+ sql1
						+ sql2
						+ sql3
						+ sql4
			...
			while (rs.next()) {
				...
				temp.put("saleamount", rs.getInt(3));
